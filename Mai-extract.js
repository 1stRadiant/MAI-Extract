const wordTypesMap = {
    ...
    'definations': ['A ', 'Definition:', 'Meaning:', 'Interpretation:', 'Explanation:', 'refers to', 'is defined as', 'is characterized by', 'is explained as', 'describes', 'denotes', 'in other words', 'can be described as', 'can be defined as', 'defined by', 'expresses', 'signifies', 'is termed as', 'implies', 'encompasses'],
    'informative_sentences': ['The', 'This', 'may', 'will', 'what happens is', 'Therefore,', 'Hence,', 'Consequently,', 'As a result,', 'In conclusion,','In ', 'This leads to,', 'In essence,', 'Thus,', 'In summary,', 'For instance,', 'To illustrate,', 'So,', 'As such,', 'In light of this,', 'This implies that,', 'In effect,', 'One possible outcome is,', 'if,', 'when,', 'while,', 'although,', 'Moreover,', 'Furthermore,', 'However,', 'Additionally,', 'On the other hand,', 'In contrast,', 'In addition to,', 'To summarize,', 'Not only that,', 'Another aspect is,', 'Considering,', 'Given that,', 'As a consequence,', 'In a similar fashion,','Please note that', 'This will'],
...
function mregex(...wordTypes) {
  const wordTypeRegex = wordTypes.map(type => {
  ...
  const firstElementRegex = `\\b(${wordTypeRegex.split('|')[0]})\\b`;
  const restOfElementsRegex = `(${wordTypeRegex.replace(/\?/g, '\\?').replace(/\|/g, '|\\s*')})`;

  const regexPattern = `(${firstElementRegex}|${restOfElementsRegex})`;
...
function getSentencesByWordTypeFromMapInParagraph(paragraph, wordType) {
    ...
    words.forEach(word => {
        const regex = new RegExp(`\\b${word}\\b`, 'g'); // Removed 'i' flag for exact case match
